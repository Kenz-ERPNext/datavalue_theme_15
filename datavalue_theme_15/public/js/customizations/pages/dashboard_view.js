...
frappe.pages["dashboard-view"].on_page_load = function (wrapper) {
    ...
    frappe.dashboard = new DataValueDashboard(wrapper);
    $(wrapper).bind("show", function () {
        frappe.dashboard.show();
    });
};

class DataValueDashboard {
    constructor(wrapper) {
    ...
    refresh() {
        frappe.run_serially([() => this.render_cards(), () => this.render_charts(), () => this.render_maps()]);
    }
    ...
    render_maps() {
        return this.get_permitted_items(
            "datavalue_theme_15.api.get_permitted_maps"
        ).then((maps) => {
            if (!maps.length) {
                return;
            }
            frappe.dashboard_utils.get_dashboard_settings().then((settings) => {
                let map_config = settings.map_config ? JSON.parse(settings.map_config) : {};
                this.maps = maps.map((map) => {
                    return {
                        map_name: map.map,
                        label: map.map,
                        chart_settings: map_config[map.map] || {},
                        ...map,
                    };
                });

                this.maps_group = new frappe.widget.WidgetGroup({
                    title: null,
                    container: this.container,
                    type: "map",
                    columns: 2,
    ...
    set_dropdown() {
        ...
        this.page.add_menu_item(__("Refresh All"), () => {
            ...
            this.maps_group && this.maps_group.widgets_list.forEach((map) => map.refresh());
        });
